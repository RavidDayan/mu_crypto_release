  ...
  )
{
  ...
  EVP_PKEY      *Pkey = NULL;  // Initialize to NULL to avoid using garbage pointer
  CONST CHAR8   *AlgName;
  ...
  //
  PkeyCtx = EVP_PKEY_CTX_new_from_name (NULL, "ML-DSA-87", NULL);
  if (PkeyCtx == NULL) {
    return NULL;
  }
...
/**
  ...
  in case MlDsaContext,PublicKeySize or PrivateKeySize is NULL, then return FALSE.
  ...
  )
{
  ...
  //
  // Check buffer sizes - must check BOTH before returning
  //
  SizeError = FALSE;
  ...
  )
{
  ...
  // NOTE: We do NOT free the old key here - caller is responsible for cleanup
  // The old key pointer is lost when we update MlDsaContext, so caller should
  // save it before calling this function if they need to free it.
  //
